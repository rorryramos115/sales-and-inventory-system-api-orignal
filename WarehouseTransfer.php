    ...
    function receiveTransfer($json) {
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                $existingStockSql = "SELECT stock_id, unit_price FROM warehouse_stock 
                                    WHERE warehouse_id = :warehouse_id AND product_id = :product_id";
                ...
                if ($existingStock) {
                    // Calculate weighted average unit price if stock already exists
                    $existingQty = $existingStockStmt->rowCount() ? $existingStock['quantity'] : 0;
                    $existingValue = $existingQty * $existingStock['unit_price'];
                    $newValue = $receivedQty * $item['unit_price'];
                    
                    $totalQty = $existingQty + $receivedQty;
